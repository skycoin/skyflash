...
win-travis: clean win-flasher-travis ## Create the windows static app via travis
	py -m PyInstaller skyflash-gui.spec
...
win-flasher-travis: ## Create the windows flasher utility inside travis windows machine
	cd win-build && py -m PyInstaller -F flash.spec
